...
class SimpleCache:
    ...
    def __init__(self, max_size: int = 1000, default_ttl: int = 300):
        ...
        # Statistics
        self.stats = {"hits": 0, "misses": 0, "evictions": 0, "expired_entries": 0}
    ...
    def set(self, key_parts: tuple, value: Any, ttl: Optional[int] = None) -> None:
        ...
        # Create cache entry
        entry = CacheEntry(
            data=value, created_at=current_time, expires_at=current_time + ttl
        )
...
def cache_api_status(ttl: int = 60):
    ...
    return cached_api_call(
        ttl=ttl, key_generator=lambda *args, **kwargs: ("api_status",)
    )
...
def cache_api_credits(ttl: int = 300):
    ...
    return cached_api_call(
        ttl=ttl, key_generator=lambda *args, **kwargs: ("api_credits",)
    )
