...
class TestAPIToDatabaseIntegration:
    ...
    def test_fetch_and_store_coins_success(
        self, mock_post, mock_influxdb, sample_coin_data
    ):
    ...
    @patch("src.lcw_fetcher.database.influx_client.BaseInfluxDBClient")
    @patch("requests.Session.post")
    def test_fetch_and_store_exchanges_success(
        self, mock_post, mock_influxdb, sample_exchange_data
    ):
    ...
    @patch("src.lcw_fetcher.database.influx_client.BaseInfluxDBClient")
    @patch("requests.Session.post")
    def test_fetch_and_store_market_data_success(
        self, mock_post, mock_influxdb, sample_market_data
    ):
...
class TestDataFetcherIntegration:
    ...
    @patch("src.lcw_fetcher.database.influx_client.BaseInfluxDBClient")
    @patch("requests.Session.post")
    def test_data_fetcher_complete_cycle(
        self,
        mock_post,
        mock_influxdb,
        sample_coin_data,
...
class TestDataConsistency:
    ...
    def test_data_transformation_integrity(
        ...
        test_data = {
            ...
            "cap": 850000000000.0,
            "rank": 1,
            "delta": {"hour": 0.5, "day": 2.3, "week": -1.2},
        }
