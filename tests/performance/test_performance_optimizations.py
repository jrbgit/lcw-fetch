...
from lcw_fetcher.utils.performance_logger import get_performance_stats, track_performance
from lcw_fetcher.utils.cache import get_cache_stats, clear_cache
...
class PerformanceValidator:
    ...
    def test_api_client_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                results['performance_metrics']['status_check_time'] = time.time() - start_time
                print(f"    ✅ Status check: {results['performance_metrics']['status_check_time']:.2f}s")
    ...
    def test_database_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"    ✅ Database connection: {results['performance_metrics']['connection_time']:.2f}s")
    ...
    def test_caching_effectiveness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if second_call_time < first_call_time * 0.5:  # 50% improvement expected
                ...
                print(f"    ✅ Cache performance: {first_call_time:.3f}s → {second_call_time:.3f}s")
            else:
                print(f"    ⚠️  Cache performance: {first_call_time:.3f}s → {second_call_time:.3f}s")
            ...
            if cache_stats['total_requests'] > 0:
                print(f"    ✅ Cache statistics: {cache_stats['hit_rate_percent']:.1f}% hit rate")
    ...
    def test_full_cycle_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cycle_times:
                results['performance_metrics']['avg_cycle_time'] = statistics.mean(cycle_times)
                results['performance_metrics']['min_cycle_time'] = min(cycle_times)
                results['performance_metrics']['max_cycle_time'] = max(cycle_times)
                
                print(f"    ✅ Average cycle time: {results['performance_metrics']['avg_cycle_time']:.2f}s")
    ...
    def test_performance_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'error' not in perf_stats:
                ...
                print(f"    ✅ Performance stats collected: {perf_stats.get('count', 0)} operations")
            else:
    ...
    def generate_summary(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if api_results.get('timeout_handling') and api_results.get('retry_logic'):
            ...
            summary['optimizations_verified'].append("Enhanced API client with timeouts and retries")
        ...
        if cache_results.get('cache_implementation') and cache_results.get('hit_rate_improvement'):
            summary['passed_tests'] += 1
            summary['optimizations_verified'].append("Response caching with hit rate improvement")
            print("  ✅ Caching System: VERIFIED")
        else:
            summary['warnings'] += 1
            summary['recommendations'].append("Monitor cache hit rates and tune TTL values")
        ...
        if monitoring_results.get('tracking_active'):
            summary['passed_tests'] += 1
            summary['optimizations_verified'].append("Performance monitoring and metrics collection")
