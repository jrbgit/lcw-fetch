...
def api_error_responses():
    ...
    return {
        "rate_limit": {
            "status_code": 429,
            "response": {
                "error": {"code": 429, "description": "API rate limit exceeded"}
            },
        },
        "auth_error": {
            "status_code": 401,
        ...
        "bad_request": {
            "status_code": 400,
            "response": {
                "error": {"code": 400, "description": "Bad request parameters"}
            },
        },
    }
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(
            keyword in item.name.lower() for keyword in ["network", "api", "database"]
        ):
